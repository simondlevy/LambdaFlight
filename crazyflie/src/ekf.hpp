        ...
        void predict(const uint32_t nowMsec, const bool isFlying)
        {
            ...
            if (nowMsec >= _nextPredictionMsec) {
                ...
                set(x_predicted, STATE_DX,
                        get(_x, STATE_DX) +
                        dt * (accx + _gyro.z * tmpSDY - _gyro.y * tmpSDZ - MSS_TO_GS * _r.x));
                ...
                const auto ze0  = (get(x_predicted, STATE_DY)*_r.z - get(x_predicted, STATE_DZ)*_r.y)*dt;
                const auto ze1  = (- get(x_predicted, STATE_DX)*_r.z + get(x_predicted, STATE_DZ)*_r.x)*dt;
                const auto ze2  = (get(x_predicted, STATE_DX)*_r.y - get(x_predicted, STATE_DY)*_r.x)*dt;
